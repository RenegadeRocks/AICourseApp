...
2. Copy `macos/ai-pro-course.30s.sh` to `~/Library/Application Support/SwiftBar/`.
3. `chmod +x` the file.
...
Create a shortcut to `windows/open-aiprocourse.bat`. Pin it to the taskbar.

### Option B — persistent system-tray icon

Install [AutoHotkey v2](https://www.autohotkey.com/), then run `windows/AIProCourse-Tray.ahk`. This creates a persistent tray icon with right-click menu (Open today, Open schedule, Start dev server, Stop dev server, Open repo).
