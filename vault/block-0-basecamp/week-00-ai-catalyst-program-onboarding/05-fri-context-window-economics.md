...
**The Anthropic >200K surcharge that came and went.** Through 2025 the 1M Opus 4.6 *beta* carried a 2×/1.5× input/output premium on tokens above 200K — operationally a message: "don't casually cross 200K." That surcharge was removed at GA on 2026-03-13, and the current lineup (Fable 5, Opus 4.8, Sonnet 5) has never had it: a 900K request is the same per-token rate as a 9K one.[^3] The trap is stale cost models — teams still assuming a 2× cliff that no longer exists, and, conversely, teams forgetting that Gemini's 200K step is very much alive. Re-derive from the current pricing page, not from a 2025 article.

## Common mistakes AI-pro leads still make

1. **Quoting input price as *the* price.** You will read a post claiming "Gemini is 2× cheaper than Opus." That's true on input under 200K. On a workflow heavy on output (code generation, long-form writing, multi-turn agents) the gap is much smaller. On a workflow with stable cacheable prefixes, it can invert. And it ignores the ~30% tokenizer tax on current Claude models. Never cite one-number-per-model comparisons.
