...
date_due: 2026-05-17
tags: [synthesis, quiz, flashcards, jtbd, fit-rubric, scoping, pricing, klarna, decagon, cursor, claude-code, devin, codex, metr, rice, eval-gates, unit-economics]
...
- **Fri — Support case study.** The four-step pipeline run end-to-end on Klarna / Fin (formerly Intercom, being acquired by Salesforce) / Decagon / Sierra, with real numbers, a public walk-back, and a 2026 consolidation wave.
- **Sat — Coding case study.** The pipeline run end-to-end on Cursor (now SpaceX-owned) / Claude Code / Copilot / Devin / Codex, with a live controversy (the METR slowdown and its stranger 2026 follow-up) and a benchmark-vs-production gap (SWE-bench Verified → Pro).
...
| Fri — Support-agent teardown (Klarna, Fin, Decagon, Sierra) | Block 2 voice-first products; any CX/CS client engagement; vendor teardowns of any agent category |
| Sat — Coding-agent teardown (Cursor, Claude Code, Copilot, Devin, Codex) | Every tool-selection conversation; Weeks 13–14 on agent evaluation; your own stack choices this year |
...
| 8 | Gate RAG on chunk-level P@5 ≥ 0.8, Recall@20 ≥ 0.95, grounding rate ≥ 0.9 at week 4 | Document-level precision lies — a retriever that finds the right doc but wrong chunk still produces hallucinations. The MIT NANDA "95% of pilots show no measurable P&L impact" figure (interrogated in [[03-wed-scoping-ai-projects|Wednesday's lesson]]) is dominated by teams who never gated retrieval before generation — but don't fuse it with any "80% RAG failure rate," which is a separate practitioner claim | Any RAG-based project | Non-retrieval generation (pure summarization, drafting, classification) |
...
| 14 | Discount SWE-bench (now Pro, not Verified) by 20–30 points when predicting in-codebase performance, and assume the agent runs at 50–60% in your messy reality | SWE-bench Pro runs well below Verified on the same model tier (frontier ~59% on Scale's public set vs 80%+ on Verified); contamination, environment simplicity, and "resolved = tests pass" are the three gaps. METR's 2025 RCT found a 19% *slowdown* on senior devs' own repos — and its Feb 2026 update found a small positive but with the control group refusing to work AI-free | Any coding-agent procurement or client recommendation | Triage — headline benchmark is still a reasonable first filter |
...
**Q8** — Sixteen experienced OSS developers, ~5 years of prior experience on their own repos, completed 246 tasks, randomly assigned AI-allowed (mostly Cursor Pro + Claude 3.5/3.7 Sonnet) or AI-disallowed. Pre-study forecast: 24% speedup. Post-study self-estimate: 20% speedup. **Measured result: 19% slowdown.** Everyone — developers, ML researchers, economists — predicted in the wrong direction. *2026 follow-up (Feb 24 2026):* the replication gave an unreliable signal because 30–50% of developers refused to submit tasks without AI; METR's latest RCT on late-2025 agents found a small (~4–20%) positive it calls weak evidence. The −19% is the honest 2025 number for Job B on mature codebases; it is not the last word.
...
**Q20** — Split by task type. Position "agents slow seniors" is right for Job B (pair-programming) *on mature codebases the developer already knows deeply* — this is exactly METR's 2025 experimental setup and the 19% slowdown is the honest measure there. Position "25–55% speedup" is directionally right for Job C (delegated tickets) on well-scoped novel work where the agent isn't competing against existing context in the developer's head. The 2026 evidence sharpens both: METR's Feb 2026 update found a small positive (~4–20%) on newer agents but with the control group collapsing (30–50% of devs refuse to work AI-free), so the clean −19% no longer holds and the honest number is "small and hard to measure." Cursor's 170%+ NDR and Claude Code's $2.5B annualized ramp reflect real willingness-to-pay, but willingness-to-pay is not measurable-throughput-gain. Honest 2026 briefing: expect subjective acceleration, modest-and-improving measurable gains on novel work, and a measurement problem on deep-context work because seniors now refuse the no-AI baseline. Any vendor claim of a precise speedup percentage is marketing.
...
14. Q: MIT NANDA State of AI in Business 2025 headline (and the caveat)? → A: ~95% of enterprise GenAI pilots produce *no measurable P&L impact yet* — traces to scope + integration, not model quality. Evidence base: 300 pilots / 150 interviews / 350 surveys; "no measurable impact" ≠ "failed," and it is *not* an "80% RAG failure rate." Canonical treatment: [[03-wed-scoping-ai-projects|Wednesday's lesson]].
...
19. Q: Harvey pricing (2026)? → A: Banded per-lawyer — ~$100–200/seat/mo at Am Law 200+-seat scale, ~$1,000–2,000/seat/mo mid-market; 25–50-seat minimums; contracts $50k–$250k+. Anchored to customer billing rate, not Harvey COGS. ($11B valuation, ~$190M ARR end-2025.) The old flat "$1,200/seat, 20-seat min, $288k floor" is superseded.
20. Q: Cursor June 2025 pricing episode — and how it ended? → A: Shifted Pro from "500 fast + unlimited slow" to "$20 of API credits at list"; new agent models burned credits 3–10× faster; backlash → CEO apology + refunds. Revenue $1B (Dec 2025) → ~$4B annualized (~$2.6B enterprise, June 2026), then **SpaceX acquired Anysphere for $60B all-stock (June 16 2026)** — largest venture-backed-startup acquisition ever.
21. Q: Claude Code unit economics (and why the "10B tokens for $1,600" anecdote is now historical)? → A: ~$13/active-dev/active-day; $150–$250/active-dev/month; Max 20x at $200/mo; default now Sonnet 5, 1M ctx; Opus 4.8 at $5/$25 (not $15/$75). The old "10B tokens ≈ $15k API for $1.6k flat, 150× subsidy" anecdote is historical: through 2026 Anthropic imposed weekly compute caps and *announced then paused* (June 15) metering programmatic Agent-SDK use at API rates. Meanwhile Cursor (Jun 2025) and Copilot (Jun 1 2026) fully flipped to usage-based — confirming Thursday's trilemma thesis.
...
25. Q: Sierra's outcome-pricing twist? → A: Charge only when AI resolves; human escalation free. Voice-native support (ADT, Sonos). $950M Series E at **$15.8B valuation (May 2026)**; past **$150M ARR in eight quarters, ~$200M in 2026** — a ~79x multiple, down from ~100x and falling.
...
27. Q: METR RCT finding — 2025 and the 2026 update? → A: 2025: 16 senior devs, 246 tasks, own repos, Cursor Pro + Claude 3.5/3.7; forecast +24%, self-estimate +20%, **measured −19%**. **Feb 2026 update:** replication gave an unreliable signal because 30–50% of devs refused to work AI-free; latest RCT on late-2025 agents found a small (~4–20%) positive it calls weak evidence. Honest read: −19% for Job B on mature codebases in 2025; small-and-improving-but-hard-to-measure in 2026.
28. Q: SWE-bench Verified → Pro gap? → A: The reported benchmark migrated from Verified (topped 80%, contaminated) to SWE-bench Pro, where frontier models sit ~59% on Scale's public set (Opus 4.8 ~69% on vendor aggregates — name the split). Causes: contamination (post-June-2024 training sees solutions), harder/multi-language task distribution, and "resolved = tests pass" ≠ correctness.
29. Q: Devin / Cognition 2026 status? → A: Devin 2.0 pricing: $20/mo + $2.25/ACU (1 ACU ≈ 15 min), enterprise VPC custom. Cognition acquired Windsurf (Jul 2025), raised $1B+ at **$26B (May 2026)**, ARR ~$492M (up 13×), and rebranded Windsurf as **"Devin Desktop"** (Jun 2 2026) — a top-three coding-agent player, not a failed premium experiment.
...
- Decagon Stripe case study + funding: $131M at $1.5B (Jun 2025) → $250M Series D at $4.5B (Jan 2026). (Fri)
- Sierra (Bret Taylor) outcome-pricing thesis — $4.5B (2024) → $10B (Sep 2025) → $950M Series E at $15.8B (May 2026), ~$200M ARR. (Fri)
- Consolidation wave (Fri/Sat): Salesforce–Fin (~$3.6B, Jun 2026), SpaceX–Cursor ($60B, Jun 2026), Cognition $26B / Windsurf → Devin Desktop, OpenAI Codex 5M weekly users + 2026 Gartner MQ. (Fri, Sat)
