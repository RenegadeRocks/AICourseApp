...
The contract clauses that earn their keep: a termination-for-convenience right with 60–90 day notice, a data-portability clause requiring export of prompts, logs, embeddings, fine-tunes in open formats, a 90-day transition-support obligation, and ownership of fine-tuned weights when the buyer supplied the training data. Methodmi's 2026 analysis of vendor lock-in in procurement platforms argues those four clauses together are what makes "exit ramp" more than rhetoric.[^13]

Owner: shared, and named. The builder owns model-provider risk monitoring; the buyer owns data-provider and internal-system risk monitoring. The engagement owns the contract clauses that make each side's monitoring actionable.
...
The cleanest for buyer and seller. Each phase has a fixed fee, a fixed duration, and fixed deliverables. Payment is on gate-passage. Advantages: predictable for both sides, no token-cost surprises for buyer, no scope-creep erosion for seller. Failure mode: a flat fee that assumed certain data access, vendor pricing, or token volumes collapses when any of those shift. A 30% swing in frontier-model API pricing during 2024–25 was not unusual (the OpenAI GPT-4-Turbo repricing, the Anthropic Claude 3.5 generation, the Google Gemini tier changes all produced >20% moves within quarters). A flat-fee-per-phase engagement that didn't include a token-cost pass-through clause either ate the swing (seller) or triggered a change order that felt adversarial (buyer).
