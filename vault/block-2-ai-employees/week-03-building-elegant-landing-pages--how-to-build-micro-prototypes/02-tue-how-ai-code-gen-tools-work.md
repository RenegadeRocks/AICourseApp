...
Yesterday's lesson stripped a landing page to its measurable anatomy. See [[01-mon-landing-page-as-conversion-machine]] for the seven-element structure this lesson assumes you can read. Today is the mechanics of the machine that assembles that anatomy at 4 AM when a hypothesis is still warm.

The substrate has moved fast. At Vercel Ship 2026 (London, June 17), Guillermo Rauch reported that coding agents now trigger **more than half of all Vercel deployments — up from under 3% six months earlier — with AI Gateway token volume growing from roughly 2 trillion to 20 trillion per month** over the same window.[^1] (The February 2026 "Agentic Infrastructure" post that put the figure at "over 30%… Claude Code at ~75% of agent deployments" is the number this lesson was originally anchored to; it is Vercel-stated internal data, so treat the trajectory as directional, not audited.) The four tools this lesson dissects have all re-priced, re-platformed, or re-modeled since then, so we teach the *architectures* — which are durable — and date-stamp every number, because the numbers rot in weeks.

The current commercial scoreboard, web-verified July 2026: v0 moved to token-metered credits and became a full-stack agentic platform.[^2] Lovable crossed **$500M annualized revenue in June 2026** (up from ~$400M in February) and is in talks to raise ~$300M at a **$13.2B valuation**, roughly double its December 2025 Series B mark.[^3] Replit hit **~$525M annualized revenue by April 2026** and raised $400M at a **$9B valuation** in March, on the back of Agent 3.[^4] Bolt.new — which went near-death to ~$40M ARR in five months through early 2025 — now runs on a Standard/Max agent model and has a hard **August 3, 2026 cutoff** for legacy v1 projects that any spring-2026 builder needs to know about.[^5][^16]
...
**Public facts, verified:** v0.dev launched October 2023 and went generally available in 2024. In May 2025 Vercel shipped `v0-1.0-md`, a model accessible through an OpenAI-compatible API, making generative UI a `curl` call from Cursor, Claude Code, or any other agent.[^2] The **February 2026 rebuild** turned v0 from a frontend-scaffolder into a full agentic platform: a sandbox-based runtime that directly imports GitHub repositories, **database connectivity and Supabase CRUD, full Next.js apps including API routes and Server Actions, a VS Code-style editor, and multi-step agentic workflows** (v0 now searches the web, inspects live sites, and debugs autonomously).[^1][^6] Pricing moved to **token-metered credits** — Free (with ~$5 monthly credits, 7 messages/day), Premium $20/mo, Team $30/user, Business $100/user — with generations drawing down credits at per-model token rates (v0 Mini $1/$5, Pro $3/$15, Max $5/$25, Max Fast $30/$150 per Mtok), plus a new **Vercel Agent (Private Beta, July 2026)**.[^2] The component substrate is still Next.js + Tailwind + shadcn/ui + React Server Components (RSC).[^7]

**Architectural claim:** v0's differentiator is not the model — multiple vendors use the same frontier models — it is the **component vocabulary and the RSC streaming primitive** Vercel open-sourced with AI SDK 3.0 in March 2024,[^7] now wrapped in the Vercel infrastructure knowledge graph (env vars, deploy config, repo context injected into every prompt). The generative-UI abstraction lets an LLM response stream React components, not text chunks, and v0 is the canonical implementation against the shadcn/ui vocabulary. Behaviorally: v0 produces components *typed* against shadcn primitives (Button, Card, Dialog, with data-slot attributes after the Tailwind v4 migration in February 2025[^8]), which integrate cleanly into projects that have shadcn installed, and break noisily in projects that do not.

**Where it wins:** single-page component-dense landing pages where the target aesthetic is within one standard deviation of Vercel Geist and deployment is on Vercel — but as of the 2026 rebuild, also **full-stack surfaces with real persistence** (Supabase CRUD, API routes), which used to be Lovable's cell in the matrix. The 70% case for AI-services landing pages, now with a backend when you need one.

**Where it fails:** deeply custom design systems deviating from shadcn's tokens still require override CSS v0's defaults do not account for. Credit spend is now the sharp edge — a Max-Fast taste-override loop meters real dollars where Claude Code on a subscription meters none. Non-React frameworks remain out of scope.
...
**Architectural claim:** Lovable's differentiator is not the UI library — it uses the same Tailwind + shadcn substrate as v0 — it is the **full-stack scaffolding pass that treats Supabase as a first-class dependency**, plus whole-team credit pooling as its commercial wedge. A Lovable prompt mentioning "users, authentication, save their preferences" produces the Supabase schema, RLS policies aligned to Supabase Auth, the React hook, and the component wiring in one pass. This used to be the clean divider from v0 ("the same prompt to v0 returns a static UI and a todo item") — but v0's February 2026 rebuild added Supabase CRUD and API routes, so the division of labor is now narrower: Lovable still wins on *managed* backend provisioning and non-technical-client editability, not on the mere existence of persistence.

**Where it wins:** prototypes that need a real backend and a client-editable surface by EOD — waitlist pages with persisted email lists and auth-gated previews, concierge intake forms with persistent state, small CRUD tools a non-engineer will maintain. The hidden win: the generated Supabase schema is usable beyond the prototype.
...
**Public facts, verified:** Bolt.new launched circa October 2024 from StackBlitz, the company that spent over four years building WebContainer — a WebAssembly-based operating system that boots a Node.js environment inside a browser tab in roughly 100 milliseconds.[^5][^13] The repository is open-sourced at `github.com/stackblitz/bolt.new` (along with the community `bolt.diy` fork at `github.com/stackblitz-labs/bolt.diy`).[^14][^15] Bolt's middleware routes prompts through Anthropic models, tracks file changes, handles package installations via WebContainer, and orchestrates third-party integrations.[^13][^17] As of 2026 Bolt no longer exposes model selection: it ships **two agents, Standard (balanced) and Max (deeper reasoning), both powered by "Claude Agent" as the default**, plus MCP-server connections, in-chat AI image generation, and a Microsoft Azure/M365 partnership (May 2026).[^16] **Operator-urgent:** the legacy v1 Agent has been unselectable for new projects since April 13, 2026, and **v1 projects and the sites built from them become inaccessible after August 3, 2026** — any reader who shipped a Bolt prototype in the spring must migrate before then (manual switching preserves files and code but clears chat history).[^16]

**Architectural claim:** Bolt's moat is the **WebContainer primitive itself**, not the model or the UI library. Submit a prompt; the browser spins up a full Node environment locally; `npm install` runs; a dev server boots on a localhost URL served back into the same tab; every file edit hot-reloads. No remote VM. WebContainer executes the entire stack in-browser rather than in a remote worker.[^13] Eric Simons has described it as a four-year bet that browser coding needed an OS-level substrate, not a syntax-highlighting layer.[^5]
...
**Public facts, verified:** Replit Agent launched early access in September 2024, built on Anthropic's Claude.[^19] The tool that matters in 2026 is **Replit Agent 3, launched September 10, 2025** — a 200-minute autonomous runtime (10× Agent 2) with a live browser self-testing loop at a median ~$0.20/session, the ability to spawn specialized sub-agents, and Slack/email/Telegram integrations.[^19b] Replit's trajectory since is the clearest commercial evidence the Agent landed: **~$525M annualized revenue by April 2026** (up from ~$300M at end-2025), 50M users, and a **$400M raise at a $9B valuation in March 2026** — 3× its valuation six months prior, targeting $1B ARR by end-2026.[^4][^20] Pricing is now **credit-based tiers** (Starter free / Core ~$20–25 with ~$25 of included credits / Teams and Pro higher), not the old flat-per-checkpoint or effort-based per-run model.[^20]

**Architectural claim:** What Replit owns is the **persistent, shared workspace with built-in deploy, database, and auth** — not the model. Every project is a long-lived environment with its own PostgreSQL, object storage, and deployment target; the Agent works inside that environment, not in a disposable session. The checkpoint-diff-commit cycle means the Agent proposes a diff, you approve or reject it, accepted diffs commit to the workspace history — Agent 3 layers autonomous test-fix-rerun loops on top of that spine.
...
|---|---|---|---|---|
| v0 | RSC streaming + Vercel infra knowledge graph | Next.js + shadcn/ui + Tailwind | Supabase CRUD + API routes (since Feb 2026) | Component-dense hero, now full-stack, on Vercel stack |
| Lovable | Generate→verify→refine plan-mode loop | React + shadcn + Tailwind | Supabase (first-class, managed) + team credit pool | Client-editable prototype with auth + DB by EOD |
| Bolt.new | WebContainer (in-browser Node OS) | Framework-agnostic; Vite default | In-browser (ephemeral) or user-configured; MCP | Full-stack demo where env = runtime (mind the Aug 3 v1 cutoff) |
| Replit Agent 3 | Persistent cloud workspace + autonomous test loop | Any (React default) | Postgres + object store (managed) | Team-kept prototype with shared URL |
...
All four tools use Anthropic's Claude family as primary code-gen. Bolt now defaults to "Claude Agent"; Lovable's architecture does the same; Replit Agent committed to Claude earliest.[^16][^19][^10] v0's `v0-1.0-md` is Vercel-fine-tuned but sits within the same frontier-model family. And the model landscape underneath all four moved *twice* since this lesson was first written: **Claude Fable 5 and Mythos 5 launched June 9, 2026** as the first "Mythos-class" tier above Opus (Fable 5 = the publicly available one, SOTA on software engineering, 1M-token context, $10/$50 per Mtok), and **Claude Sonnet 5 (June 30, 2026)** reset the cheap-agent floor at $2/$10 intro pricing through Aug 31 and became the default in Claude Code.[^21][^21b] Fable 5 is itself the cleanest test case for this section's thesis: days after launch the US government suspended it under an export directive (June 12), then lifted controls and Anthropic redeployed it globally July 1 — the frontier model your tools run on can be pulled offline by the state, which is a platform-risk lesson in its own right.[^21b]

When the same model powers all four, raw generation capability converges. What one tool generates this week, another generates next week given the same model upgrade. This is the commodity race-to-zero Swyx, Mckay Wrigley, and other commentators describe on Latent Space and X.[^22] A new tool reaches parity on pure generation in roughly one model release cycle — and the Fable-5→Sonnet-5 churn of June 2026 compressed that cycle to weeks.
...
The related controversy — does AI-code-gen plateau at 80% or 95% of a shippable landing page? — splits into two camps. The last-mile-is-human camp: the final 20% (taste, last-mile overrides, responsive edges) is where the generalist tool becomes net drag, and a human design engineer adds more value than iterating prompts. The iteration-wins camp (Rauch, swyx, Rauno on shipped cases): 2025–2026 shipped production marketing surfaces went AI-generated → production via iteration, not replacement.
...
That sameness is the maximum-likelihood output of the model given "build a landing page," averaged across the training corpus — working as designed, not malfunctioning. Brian Lovin's position on taste as competitive edge lands here: in an era when anyone can generate the default, taste is one of the few remaining moats.[^27]
...
[[03-wed-design-system-literacy|Wednesday's lesson]] will deep-dive the seven variables (type, color, spacing, density, motion, imagery, voice). For today, what matters is that **they are the fastest debug surface for a wrong-looking page.** When the hero is off, the failure localizes to one or two variables, and the fix is a targeted prompt addendum, not re-generation:
...
### Case study 2 — Replit Agent 3 and the run to ~$525M

Per Kyle Poyar's *Growth Unhinged* and Sacra, Replit ended 2024 around $10M ARR, crossed $100M in mid-2025, reached ~$300M by end-2025, and hit **~$525M annualized by April 2026** on 50M users — with the March 2026 $400M raise at $9B valuing the acceleration explicitly.[^4][^20] The inflection is the Agent line — early access September 2024, then **Agent 3 (September 10, 2025)** with its 200-minute autonomous runtime and self-testing loop; the cause is that the pre-existing primitive — persistent workspaces with managed Postgres and deploy — was exactly the envelope the Claude-family model needed to become a product for non-developers.

**Operator move:** Replit moved through three pricing regimes in eighteen months — flat $0.25-per-checkpoint, then effort-based ($0.06 floor to multi-dollar), now **credit-based tiers** (included monthly credit allowances per plan).[^20] The lesson is not any one price; it is that agent-run pricing must absorb run-complexity variance, and a flat-per-unit price incentivizes the agent to hack "unit completion" and starves the user of deep work. That the model changed three times in eighteen months is itself the point — do not hard-code a competitor's price into your own unit economics.

**Transfer:** if you charge per project for AI services, your pricing model should reflect actual compute variance, not flat-rate theater — and should survive a vendor re-pricing without breaking your margins.

### Case study 3 — Lovable, from $10M-in-60-days to $500M ARR

Per Osika's 2025 deck, Lovable hit **$4M ARR in four weeks, $10M in two months with a 15-person team**, Europe's fastest-growing startup by that metric.[^3][^11] Eighteen months later the envelope thesis is winning at scale: **~$400M ARR in February 2026, $500M annualized announced June 2026 on a headcount of ~146** (roughly $3.4M ARR per employee), with the December 2025 Series B ($330M at $6.6B, CapitalG/Menlo) followed by July 2026 talks to raise ~$300M at a **$13.2B valuation**.[^3][^10] Osika documents the shift from "single-shot generator" to "managed product assembly environment" — envelope, not model, is the product.[^12]
...
1. **Will the envelope-vs-model moat hold through 2026–2027?** No longer a pure bear-case hypothetical: **Anthropic shipped Claude Design (April 17, 2026, Anthropic Labs)** — prompt-to-prototype/design/slides, powered by Claude Opus 4.7, in research preview for Pro/Max/Team/Enterprise, building a per-team design system by reading your codebase and design files — and **Claude Cowork went cross-platform (web + mobile, July 7, 2026)**, generalizing the coding-agent pattern to all knowledge work.[^30] So the disruptor the original bear case predicted is partially on the field. Bull case still standing: v0/Lovable/Bolt/Replit envelopes are opinionated enough at a specific job (WebContainer, managed Postgres, team credit pools) that a horizontal Anthropic product hasn't subsumed any of them yet. Resolution visible by late 2026 based on which envelope-tool's growth curve flattens first.

2. **Does Figma Make's multimodal pipeline beat text-to-code for design-heavy prototypes?** Figma Make is now **generally available and multi-model** — you can select the latest Claude (including Opus 4.7), Gemini, or GPT via Figma AI credits, and it supports MCP connections and design-file context.[^31] With Claude Design and Figma Make both live, Friday's "Figma Make vs v0" duel is a three-way fight. Counter (swyx camp): text is more compressible and versionable; multimodal gen introduces divergence that is harder to diff than text-prompt divergence. Unresolved.
...
- **Andrej Karpathy (Sequoia Ascent 2026 fireside on agentic engineering)**[^33] would push back on the taste-override section. His February 2025 "vibe coding" post and his formalized 2026 framing — *"vibe coding raises the floor; agentic engineering raises the ceiling"* — hold that the professional discipline is not to push generated output toward a pre-specified aesthetic through hand-written prompts, but to design specs, supervise plans, inspect diffs, and build *evaluation loops*: an agent that reads the generated page, scores it against the aesthetic, and iterates without each addendum being hand-written. His skills list (spec design, diff review, eval design) maps almost 1:1 onto this lesson's aesthetic-brief-vs-agent point. That eval-loop agent is buildable today in Claude Code.
...
- Andrej Karpathy — *Sequoia Ascent 2026 summary* (karpathy.bearblog.dev/sequoia-ascent-2026). The "vibe coding raises the floor, agentic engineering raises the ceiling" framing and skills list.
...
[^1]: Vercel Blog. *Vercel Ship 2026 recap.* June 2026. https://vercel.com/blog/vercel-ship-2026-recap — supports the claim that coding agents now trigger more than half of Vercel deployments (up from under 3% six months earlier) and that AI Gateway token volume grew from ~2T to ~20T/month. Vercel-stated internal data; corroborated by TechCrunch's July 6 2026 Rauch interview (https://techcrunch.com/2026/07/06/vercel-ceo-guillermo-rauch-on-the-fight-to-split-off-models-from-agents/). The earlier "over 30%… Claude Code ~75%" figures are from the February 2026 *Agentic Infrastructure* post (https://vercel.com/blog/agentic-infrastructure), now superseded. Verified 2026-07-17.

[^2]: Vercel Blog. *Introducing the new v0* (Feb 3 2026, https://vercel.com/blog/introducing-the-new-v0) and *Updated v0 pricing* (https://vercel.com/blog/updated-v0-pricing); v0 pricing page https://v0.app/pricing. — support the February 2026 full-stack rebuild (Supabase CRUD, API routes, VS Code-style editor, agentic workflows) and the token-metered credit pricing (Free/Premium $20/Team $30/Business $100; per-model token rates v0 Mini/Pro/Max/Max Fast) plus the Vercel Agent Private Beta. Verified 2026-07-17.

[^3]: TechCrunch, *Lovable reportedly in talks to double its valuation to $13.2B* (July 8 2026, https://techcrunch.com/2026/07/08/lovable-reportedly-in-talks-to-double-its-valuation-to-13-2b/); Forbes, *Lovable in talks to raise funding at a $12 billion valuation* (June 5 2026, https://www.forbes.com/sites/rashishrivastava/2026/06/05/ai-coding-startup-lovable-in-talks-to-raise-funding-at-a-12-billion-valuation/); Lovable, *$10M ARR in 60 days with 15 people* (2025 deck, https://lovable.dev/video/building-lovable-10m-arr-in-60-days-with-15-people-anton-osika-ceo-and-co-founder). — support $4M ARR in four weeks / $10M in 60 days (2025); ~$400M ARR Feb 2026; $500M annualized June 2026 at ~146 headcount; Dec 2025 Series B $330M at $6.6B; July 2026 talks at $13.2B. Verified 2026-07-17.

[^4]: Kyle Poyar, *Inside Replit's path to $100M ARR*, Growth Unhinged, March 19 2025. https://www.growthunhinged.com/p/replit-growth-journey — supports the early-2025 path to $100M ARR and 45% MoM subscriber growth post-Agent (byline is Kyle Poyar, whose newsletter this is; the earlier "Jason Donahue" attribution was an error). TechCrunch, *Replit snags $9B valuation six months after hitting $3B* (March 11 2026, https://techcrunch.com/2026/03/11/replit-snags-9b-valuation-6-months-after-hitting-3b/) and Sacra (https://sacra.com/c/replit/) — support ~$300M end-2025 → ~$525M annualized by April 2026, 50M users, $400M raise at $9B, $1B ARR target by end-2026. Verified 2026-07-17.
...
[^16]: Bolt.new Release Notes. https://support.bolt.new/release-notes — supports the current Standard/Max two-agent model (both default to "Claude Agent"), MCP-server connections, in-chat AI image generation, the Microsoft Azure/M365 partnership (May 2026), and the operator-critical timeline: v1 Agent unselectable for new projects since April 13 2026; **v1 projects and their sites inaccessible after August 3 2026** (manual switch preserves files/code, clears chat history). Verified 2026-07-17.
...
[^19b]: Replit Blog / InfoQ. *Introducing Agent 3: Our Most Autonomous Agent Yet* (Sept 10 2025, https://blog.replit.com/introducing-agent-3-our-most-autonomous-agent-yet ; https://www.infoq.com/news/2025/09/replit-agent-3/). — supports Agent 3's 200-minute autonomous runtime (10× Agent 2), the browser self-testing loop at ~$0.20 median/session, sub-agent spawning, and Slack/email/Telegram integrations. Verified 2026-07-17.

[^20]: Sacra, *Replit revenue, funding & news* (https://sacra.com/c/replit/) and Espressio, *The Ultimate Replit Guide for 2026: Agent 3, Pricing & Apps* (https://espressio.ai/blog/replit-guide-2026/). — support the pricing evolution (flat $0.25-per-checkpoint → effort-based → 2026 credit-based tiers with included monthly credit allowances) and the ~$525M annualized / 50M-users / $9B figures. Verified 2026-07-17.

[^21]: Anthropic, *Introducing Claude Sonnet 5* (June 30 2026). https://www.anthropic.com/news/claude-sonnet-5 — supports Sonnet 5 as the new default in Claude Code and on Free/Pro, $2/$10 intro pricing through Aug 31 2026 (then $3/$15), 1M-token context. Verified 2026-07-17.

[^21b]: Anthropic, *Claude Fable 5 and Mythos 5* (https://www.anthropic.com/news/claude-fable-5-mythos-5) and *Redeploying Claude Fable 5* (https://www.anthropic.com/news/redeploying-fable-5); CNBC, *Trump admin lifts export controls on Fable 5 and Mythos 5* (June 30 2026, https://www.cnbc.com/2026/06/30/anthropic-says-trump-admin-has-lifted-export-controls-on-claude-fable-5-and-mythos-5.html). — support the June 9 2026 launch of the Mythos-class tier above Opus, Fable 5 at $10/$50 per Mtok with 1M-token context, the June 12 export-directive suspension, and the July 1 2026 global redeployment. Verified 2026-07-17.
...
[^30]: Anthropic, *Introducing Claude Design by Anthropic Labs* (April 17 2026, https://www.anthropic.com/news/claude-design-anthropic-labs) and TechCrunch, *Claude Cowork expands to mobile and web* (July 7 2026, https://techcrunch.com/2026/07/07/the-coding-agent-wars-are-spilling-into-the-rest-of-the-office-claude-cowork/). — support Claude Design (prompt-to-prototype/design/slides, powered by Opus 4.7, research preview on paid tiers, builds a per-team design system from your codebase) and Claude Cowork's July 2026 web+mobile launch. Replaces the earlier kingy.ai leak citation, which is now obsolete. Verified 2026-07-17.

[^31]: Figma Help Center, *Select an AI model to use in Figma Make* (https://help.figma.com/hc/en-us/articles/36400680326551-Select-an-AI-model-to-use-in-Figma-Make) and *How AI credits work* (https://help.figma.com/hc/en-us/articles/33459875669015-How-AI-credits-work). — support Figma Make's general availability and multi-model selection (latest Claude incl. Opus 4.7, Gemini, GPT) via Figma AI credits. Verified 2026-07-17.
