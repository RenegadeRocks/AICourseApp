...
Every team shipping a "chat with our docs" product in 2026 is running the same five-box diagram: chunker → embedder → vector store → (optional) reranker → generator. The boxes are so standard that LangChain, LlamaIndex, Haystack, Vercel AI SDK, and every vendor SDK ship them as a default tutorial on the front page. That's the problem. Default-RAG lands, as an *operator rule of thumb* (not a benchmark — treat these two numbers as folklore, and measure your own), roughly 60–75% retrieval hit-rate on enterprise corpora and about 50–65% end-to-end answer correctness — the range where "demo works" and "the product hallucinates on 4 in 10 queries" coexist.[^1] The gap between default-RAG and production-grade RAG is not a secret technique. It's a stack of ten decisions, each with measurable impact, each with operator consensus that is *not* what the defaults ship.
...
- You have built one trivial RAG pipeline. LangChain's "Chat with your PDF" or the Anthropic SDK's retrieval cookbook. **This lesson assumes [[03-wed-rag-as-a-system]] from Block 0 Week 1** — embeddings vs BM25, hybrid, chunking, Contextual Retrieval in full, reranking basics. If you read that lesson you qualify, and this Thursday is deliberately a *delta*: it does not re-teach the Contextual Retrieval number ladder (that lives in b0w1), it adds the cost math, the MTEB/reranker generation update, the RRF mechanics, and the cross-domain failure modes on top.
...
## Layer 1 — Contextual Retrieval as a cost decision (the number ladder lives in b0w1)

You met Anthropic's Contextual Retrieval in [[03-wed-rag-as-a-system]] — the full number ladder (top-20 retrieval failure 5.7% → 2.9%, a 49% reduction combining contextual embeddings with contextual BM25; 67% with a reranker; 35% for contextual embeddings alone), the mechanism (prepend a 50–100 token generated "context" to each chunk before embedding and BM25-indexing so a decontextualized "*Revenue declined 3%*" chunk gets bound to "*ACME Q2 2024, Services, EMEA*"), and Simon Willison's *"should work for any embedding model"* caveat.[^3][^4] **One-line recap: it's a preprocessing step that repairs the binding standard chunking destroys, and it works.** This lesson does not re-derive those numbers; it does the two things b0w1 didn't — the deployment cost math, and whether the benchmark transfers to your corpus.

**Scope, carried forward as a decision input.** Anthropic tested on "various knowledge domains (codebases, fiction, ArXiv papers, Science Papers)" — moderate-size technical docs in the 8k-token range.[^3] They did *not* test legal corpora (100M+ token archives with dense citation cross-references), medical literature (where chunking across section boundaries destroys clinical context), structured financial filings (10-Ks where table and narrative interact), or messy enterprise support docs. That is a scoping observation you carry into any adoption decision, not a knock on the technique.

### The cost math nobody reads in full — and the model swap hidden inside it

Anthropic's post gives a headline deployment number: "*the one-time cost to generate contextualized chunks is $1.02 per million document tokens*," assuming 800-token chunks, 8k-token documents, 50-token instructions, and 100 tokens of generated context per chunk.[^3] Read the fine print before you budget: **that $1.02 figure is computed on Claude 3 Haiku at its $0.25/M input price, with August-2024 prompt caching charging cache hits at 10% of standard input.**[^3][^5] Prompt caching is the engineering that makes the number small — without it you pay full input price on every chunk (each chunk drags the whole document along), 5–10× more.

The trap for a 2026 builder: the experiment below tells you to run the preprocessing on **Claude Haiku 4.5** ($1/M input), not Claude 3 Haiku ($0.25/M). That is a 4× jump in the per-token ingestion price before you touch the token-count math. So either (a) quote the $1.02 figure explicitly as a *Claude-3-Haiku* basis and note your Haiku 4.5 run will cost ~4× that, or (b) recompute for Haiku 4.5 on your own chunk/document sizes. Don't quote Anthropic's $1.02 and then run the pipeline on a model 4× more expensive — that is exactly the silent-model-swap that makes a cost table lie. Two further corollaries: a provider without aggressive cache discounts flips the economics 10×; and smaller chunks (200 tokens for compliance paragraphs) or documents bigger than the model's context window change the (document tokens × chunks) / (cache-shared chunks) ratio. Run the math for your own corpus.
...
The Vectara NAACL 2025 peer-reviewed study (predecessor work started in 2024) found that on realistic document sets, fixed-size chunking *consistently outperformed* semantic chunking across document retrieval, evidence retrieval, and answer generation tasks, with the computational overhead of semantic methods not justified by the results.[^9] One 2026 vendor benchmark (Firecrawl) reports recursive character splitting at 512 tokens with 50–100 tokens of overlap scoring ~69% accuracy and outperforming every more expensive alternative — a benchmark-validated default, though a vendor's benchmark you should re-run on your own corpus.[^10] The Chroma / Firecrawl chunking evaluation suite, which tests `LLMSemanticChunker` (0.919 recall), `ClusterSemanticChunker` (0.913), and `RecursiveCharacterTextSplitter` (85.4–89.5%, best at 400 tokens: 88.1–89.5%) confirms the surprise finding: semantic chunking is competitive on recall but loses on the cost/performance Pareto.[^10]
...
The Massive Text Embedding Benchmark (MTEB) is the industry-standard retrieval benchmark — a large task suite across 8 categories, with a multilingual (MMTEB) variant spanning 100+ languages.[^14] The leaderboard at huggingface.co/spaces/mteb/leaderboard updates continuously, which is the first thing to internalize: **any table you screenshot is stale within a quarter.** Here is where the board sits as of **July 2026**, web-verified — and note that the top is now *open-weight*, a full reversal from the OpenAI/Cohere-dominated board of 2024:

| Model | Board / MTEB score | $/1M tokens | Dimensions | Context | Notes |
|---|---|---|---|---|---|
| QZhou-Embedding (Kingsoft, on Qwen2.5-7B) | **#1 English MTEB, ~75.97** | free (open-weight) | 3584 | 8K | Current MTEB leader; also #1 CMTEB[^15] |
| Tencent KaLM-Embedding-Gemma3-12B | **#1 multilingual MMTEB, 72.32** | free (open-weight) | — | 8K | Multilingual-board leader[^15] |
| Qwen3-Embedding (Alibaba) | top-tier; open-weight MTEB leader in early 2026 | free (open-weight) | up to 4096 (MRL) | 32K | Outranks Gemini Embedding; +~6 MTEB pts over OpenAI[^15] |
| Google Gemini Embedding | strong commercial, no longer #1 (~68 band) | varies | 3072 | 8K | Multimodal; was the English #1 in early 2026[^15] |
| Voyage voyage-3-large ("Voyage AI by MongoDB") | "#1 across 8 domains, 100 datasets" at Jan-2025 launch[^16] | ~$0.18 (first 200M free)[^16] | 2048/1024/512/256 (MRL) | 32K | Strong commercial generalist |
| Cohere embed-v4 | ~65 | $0.12 | 1536 | 128K | Text+image native, multimodal[^17] |
| OpenAI text-embedding-3-large | ~64.6 | $0.13 | 3072 (or truncated) | 8K | Legacy industry default[^18] |
| OpenAI text-embedding-3-small | ~62.3 | $0.02 | 1536 (or truncated) | 8K | Cheap baseline[^18] |
| Voyage voyage-3 | ~63 | $0.06 | 1024 | 32K | Half the cost of OpenAI large[^19] |

The story the leaderboard tells that wasn't true two years ago: **open-weight models now own the top of the board outright.** As of July 2026 the English MTEB #1 is QZhou-Embedding (~75.97, built on Qwen2.5-7B), the multilingual MMTEB #1 is Tencent's KaLM-Embedding-Gemma3-12B (72.32), and Alibaba's Qwen3-Embedding line outranks Google's Gemini Embedding — all free, open-weight, and Chinese-lab-authored.[^15] Google's Gemini Embedding held the English #1 spot as recently as early 2026 (~68 band) and has since been passed; OpenAI and Cohere, who occupied most of the top in 2024, are now mid-table. Voyage (rebranded **"Voyage AI by MongoDB"** after the February 2025 acquisition) holds a strong *commercial* niche — voyage-3 at $0.06/M tokens delivering quality roughly comparable to OpenAI's $0.13/M product, and voyage-3-large ranking "#1 across eight evaluated domains spanning 100 datasets" at its January 2025 launch.[^16][^19] The scores above sit in different bands (the ~62–68 numbers are an earlier leaderboard era; the ~72–76 numbers are the current top) — and the board conflates English-MTEB and multilingual-MMTEB unless you check which board each row is from. Which is the whole point of Layer 3's operator rule below.
...
- If you're deploying on-prem, budget-constrained, or want open-weight: the open-weight tier now *is* the leaderboard top — Qwen3-Embedding (32K context, MRL dimensions), QZhou-Embedding, or the KaLM-Embedding-Gemma3 line for multilingual; BGE-M3 and nomic-embed-text v1.5 remain solid lighter-weight options.[^15][^14]
...
**Cross-encoder rerankers (Voyage rerank-2.5, Cohere Rerank 4, Jina Reranker v2).** Encode (query, document) as a single input, compute a relevance score. High quality, high latency per candidate, doesn't scale beyond ~100 candidates in real-time budgets. The current generation is a step past what most tutorials show: Voyage's **rerank-2.5 / 2.5-lite (August 2025)** are the first *instruction-following* rerankers — you steer relevance in natural language ("prefer primary sources," "rank legal precedents above commentary") — with a 32K context window (8× Cohere Rerank v3.5) and, per Voyage's 93-dataset suite, +7.94% / +7.16% accuracy over Cohere Rerank v3.5; Cohere shipped **Rerank 4** (rerank-4-pro / -fast, 32K context) in December 2025.[^25][^26]
...
### The reranker generation, 2024 → 2026

The generation matters more than most RAG writeups admit, because the numbers moved twice. Voyage's rerank-2 (September 2024) was the release-time benchmark — +13.89% over OpenAI v3 large embeddings, +15.61% over Cohere v3 on Voyage's internal suite; rerank-2-lite traded a little quality for latency.[^25] But that is now two generations back. **Voyage rerank-2.5 / 2.5-lite (August 2025)** are the current Voyage flagship: instruction-following, 32K context, +7.94% / +7.16% over Cohere Rerank v3.5 across 93 datasets and +12.70% on the MAIR instructed-retrieval benchmark.[^25] On the Cohere side, Rerank 3 (April 2024, 100+ languages, 4K context) and Rerank 3.5 (December 2024, +26.4% cross-lingual) were superseded by **Cohere Rerank 4** (rerank-4-pro / -fast, 32K context, December 2025).[^26][^27] The Agentset rerankers leaderboard (2025) puts the fastest production-grade rerankers in the ~595–603ms average-latency band; treat exact per-model latencies as re-measure-on-your-own-stack.[^28]

If you built a RAG pipeline against "Cohere Rerank 3.5" or "Voyage rerank-2" a year ago, the instruction-following capability in the 2.5/4 generation is the upgrade most likely to move your numbers — you can now push retrieval-relevance intent (jurisdiction, recency, source-tier) into the reranker in plain language instead of pre-filtering it.
...
| + Contextual BM25 + Contextual Embeddings (full Contextual Retrieval) | 2.9% | 49% | +5 | +$1.02 (Claude-3-Haiku basis; ~4× on Haiku 4.5 — see Layer 1) |
| + reranker over top-150 (Voyage rerank-2.5 or Cohere Rerank 4) | 1.9% | 67% | +600 | +~$2/1K rerank calls |
...
The decision you're making is "which subset of interventions Pareto-dominates on my cost / latency / quality frontier given my traffic pattern" — a four-row spreadsheet per project, not a hunt for the single best intervention and not an architectural debate.
...
> "Build a baseline RAG pipeline over the corpus in `./corpus/`. Chunking: recursive character splitter, 512 tokens, 50-token overlap. Embedder: OpenAI text-embedding-3-small. Vector store: Chroma or pgvector, your pick. Retrieval: top-5 dense cosine similarity. Generator: Claude Sonnet 5 (`claude-sonnet-5`) with the standard RAG prompt. Evaluate on the 50 queries in `./queries.json`: (a) retrieval hit-rate@5 — did any retrieved chunk contain the ground-truth answer span, (b) end-to-end answer correctness via LLM-as-judge with Claude Opus 4.8 (`claude-opus-4-8` — the judge should be at least as capable as the generator) and a rubric I'll provide. Output results as a markdown table."

**Phase 2 — Four interventions, measured in isolation.**

> "Now run four interventions one at a time, starting from the Phase 1 baseline each time, and measure the delta in hit-rate@5, answer correctness, p95 latency, and ingestion cost. Intervention A: swap fixed 512/50 for recursive-by-sentence with overlap. Intervention B: add BM25 + RRF hybrid retrieval (k=60). Intervention C: add a reranker (Voyage rerank-2.5 or Cohere Rerank 4) over top-20 candidates, return top-5. Intervention D: apply Anthropic's Contextual Retrieval preprocessing — use Claude Haiku 4.5 (`claude-haiku-4-5`) with prompt caching to generate 50-100 token contextual prefixes for each chunk, prepend before both BM25 and embedding. **Report the ingestion cost against Haiku 4.5's actual $1/M input price, not Anthropic's $1.02/M figure (which was computed on Claude 3 Haiku at $0.25/M — expect ~4× that).** Output a 5-column comparison table: intervention × hit-rate delta × answer-correctness delta × latency delta × cost delta."
...
[^2]: **Operator folklore, not a sourced benchmark.** The "60–75% retrieval hit-rate / 50–65% end-to-end correctness" framing numbers are a widely-repeated practitioner rule of thumb; they are *not* in the Menlo Ventures "State of Generative AI in the Enterprise" report an earlier draft of this vault attached them to. They are labeled as folklore in the body and should be treated as a hypothesis to measure on your own corpus, never quoted to a stakeholder as a benchmark. Re-labeled 2026-07-17.
...
[^10]: Firecrawl, "Best Chunking Strategies for RAG (and LLMs) in 2026" (2026). https://www.firecrawl.dev/blog/best-chunking-strategies-rag — recursive character splitting at 400–512 tokens as the benchmark-validated default (recursive-512 ~69% accuracy; 88–89% recall at 400 tokens with text-embedding-3-large); LLMSemanticChunker 0.919 recall; ClusterSemanticChunker 0.913. This is a *vendor* benchmark carrying benchmark-grade weight — quote it as "one vendor benchmark reports…" and re-measure on your own corpus. Verified 2026-07-17.
...
[^15]: MTEB leaderboard state as of July 2026, web-verified. Live board: https://huggingface.co/spaces/mteb/leaderboard. **QZhou-Embedding (Kingsoft, built on Qwen2.5-7B) leads English MTEB at ~75.97** and also tops CMTEB — technical report arxiv 2508.21632 (https://arxiv.org/abs/2508.21632), model card https://huggingface.co/Kingsoft-LLM/QZhou-Embedding. **Tencent KaLM-Embedding-Gemma3-12B is #1 on the multilingual MMTEB board at 72.32.** Alibaba's **Qwen3-Embedding** line (arxiv 2506.05176) outranks Google's Gemini Embedding (~+6 MTEB points over OpenAI). Google's Gemini Embedding held the English #1 (~68 band) as recently as early 2026 per VentureBeat, "New embedding model leaderboard shakeup" (https://venturebeat.com/ai/new-embedding-model-leaderboard-shakeup-google-takes-1-while-alibabas-open-source-alternative-closes-gap) — now superseded. Cross-checked against CodeSOTA MTEB tracker (https://www.codesota.com/benchmarks/mteb) and app.ailog.fr's 2026 MTEB writeup. Note: English-MTEB and multilingual-MMTEB are different boards — the ~62–68 scores on other rows are an earlier board era. Verified 2026-07-17.

[^16]: Voyage AI, "voyage-3-large: the new state-of-the-art general-purpose embedding model" (January 7, 2025). https://blog.voyageai.com/2025/01/07/voyage-3-large/ — Ranks first across 8 evaluated domains spanning 100 datasets. Supports output_dimension values of 2048, 1024 (default), 512, 256 via Matryoshka Representation Learning. First 200M tokens free. Note (2026-07-17): Voyage is now branded **"Voyage AI by MongoDB"** following MongoDB's February 2025 acquisition; current docs live at https://www.mongodb.com/docs/voyageai/, and blog.voyageai.com links may redirect to mongodb.com.
...
[^25]: Voyage AI by MongoDB, "rerank-2.5 and rerank-2.5-lite: instruction-following rerankers" (August 11, 2025). https://www.mongodb.com/company/blog/product-release-announcements/rerank-2-5-and-rerank-2-5-lite-instruction-following-rerankers (also https://blog.voyageai.com/2025/08/11/rerank-2-5/) — first instruction-following rerankers; 32K context (8× Cohere Rerank v3.5); +7.94% / +7.16% over Cohere Rerank v3.5 across 93 datasets, +12.70% on MAIR, +11.48% average when leveraging instructions. Supersedes rerank-2 (September 30, 2024, https://blog.voyageai.com/2024/09/30/rerank-2/, the release-time +13.89% / +15.61% numbers). Verified 2026-07-17.

[^26]: Cohere reranker line. Rerank 3 (April 2024, https://cohere.com/blog/rerank-3 — 100+ languages, 4K context) → Rerank 3.5 (December 2024, https://cohere.com/blog/rerank-3pt5 — +26.4% cross-lingual) → **Rerank 4** (rerank-4-pro / rerank-4-fast, 32K context, December 2025 — the current Cohere flagship). Verified 2026-07-17.

[^27]: Cohere, "Introducing Rerank 3.5: Precise AI Search" (December 2024). https://cohere.com/blog/rerank-3pt5 — Improved reasoning; +26.4% on cross-lingual search. Superseded by Cohere Rerank 4 (December 2025) — see [^26].

[^28]: Agentset, "Best Rerankers for RAG | Leaderboard" (2025). https://agentset.ai/rerankers — source for the ongoing reranker latency/accuracy comparison; the fastest production-grade rerankers sit in the ~595–603ms average-latency band. Treat exact per-model latencies as re-measure-on-your-own-stack; this is a blog-tier benchmark carrying benchmark-grade weight, so quote it as "one vendor benchmark reports…". Verified 2026-07-17.
