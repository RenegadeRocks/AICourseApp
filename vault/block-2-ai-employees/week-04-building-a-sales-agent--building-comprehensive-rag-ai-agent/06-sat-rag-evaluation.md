...
- Thursday's and Friday's RAG content: you know what retrieval-augmented generation is, you have a mental model for hit-rate@k vs answer correctness, and you recognize that long-context and agentic retrieval are alternatives with their own cost profiles. If you skipped those, go back.
...
The RAGAS framework, introduced by Shahul Es, Jithin James, and Luis Espinosa-Anke in their September 2023 arxiv paper *Ragas: Automated Evaluation of Retrieval Augmented Generation*,[^4] presented at EACL in March 2024, is the default "first eval harness" for RAG in 2026 because it is free, open-source, and opinionated — it picks a small set of metrics and tells you how to compute them. For an AI-catalyst lead, the value of knowing RAGAS is not "install it and trust the numbers." It is "read the metric math, understand exactly what each number measures, and know the three places each metric lies to you."
...
**Regression-gate rule, concrete version.** The operator move is to pick one number from Level 1 and one from Level 2 and write a CI rule: "block any deploy that drops the Level 1 assertion pass-rate by >2 percentage points on the 200-example regression set, or drops the Level 2 judge-scored faithfulness by >5 points, whichever is tighter." The specific thresholds — 2 points on assertions, 5 points on faithfulness — are starter defaults from conversations with shops running this in production. The exact numbers matter less than the discipline: every pipeline change that ships has gone through a gate that could have stopped it, and the gate threshold is pre-registered before the change runs, not eyeballed after.
...
**DeepEval.** Open-source evaluation framework from Confident AI, Apache 2.0, designed to feel like Pytest for LLM apps.[^19] Fifty-plus research-backed metrics including faithfulness, answer relevancy, contextual precision and recall, hallucination, bias, toxicity. The case for DeepEval: you want an eval library, not a platform, and you want your evals to live in your code repo next to your pipeline. The case against: you still need a place to put the traces from production, which DeepEval doesn't provide.
...
- *Logistics over operational runbooks.* Failure mode: the agent answers the literal question but misses the escalation path. Eval: "did the answer include the correct escalation contact for this severity level?" is a narrow rubric item a rubric-grounded judge can score >95% on with calibration.
...
> (a) A regression set of 30 queries with ground-truth answer spans, stored as a YAML file. If I don't supply one, generate it synthetically from my corpus using Claude Opus 4.7 and flag every generated item as `synthetic: true` so I can hand-review and correct before use.
> (b) Implement the three RAGAS-style metrics — faithfulness, context precision, context recall — using Claude Opus 4.7 as the judge. Save the exact judge prompts to a prompts/ directory so I can iterate them.
...
> (d) Output results as a pandas DataFrame and a markdown table, with one row per query and columns per metric, plus an aggregate row.
...
> "Take each of the four interventions from Thursday-Friday (hybrid retrieval, reranker, contextual retrieval, agentic-retrieval loop) and re-run the full harness. Produce a regression table: intervention × metric × delta from baseline × latency × cost. Then write a one-page Markdown summary titled 'Deploy decision memo' that for each intervention answers: would this pass a CI gate of 'no more than 2pp drop on any metric and no more than 1.5x latency increase and no more than 2x cost increase'? If yes, recommend ship. If no, specify the blocker."
...
- **Judge drift.** Judge model gets swapped (claude-sonnet-4-6 to claude-sonnet-4-7, say) and scores move by 3–5 points on the same regression set with no pipeline change. Mitigation: version-pin the judge in CI; treat judge-model change as a separate PR that must be accompanied by a recalibration run.
