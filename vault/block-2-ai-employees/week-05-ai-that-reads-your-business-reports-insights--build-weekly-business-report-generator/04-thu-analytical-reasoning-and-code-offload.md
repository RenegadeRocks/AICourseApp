...
**FinanceBench (Islam et al. 2023, arxiv 2311.11944).**[^1] 10,231 questions drawn from publicly-traded companies' 10-K, 10-Q, and earnings-call transcripts. 150 of those are the "open source subset" with ground-truth answers and source-page evidence strings published on Hugging Face as `PatronusAI/financebench`[^2]. The original paper's headline result — 81% refuse-or-wrong for GPT-4-Turbo with retrieval — is the number to anchor on. The current leaderboard is noisier because Patronus gate-keeps the full set, but community runs on the 150-question open subset in 2024/25 put Claude 3.5 Sonnet and GPT-4o in the 55–65% range when paired with a well-tuned retrieval system, and Claude with code execution enabled in the 70–78% range. Claim these numbers conservatively: the gap is real, the exact digits move with every prompt revision. Patronus's own documentation[^2] is the primary source for methodology; the open subset is reproducible and should be part of your own eval harness if you ship financial reports.
...
**GSM8K / GSM8K-Hard / MATH.** GSM8K is saturated at the frontier (Claude Opus 4.7, GPT-5 class, and Gemini 2.5 Pro all score 94–99%). It should no longer be cited as evidence of "LLM math ability" in any serious architecture document; the appropriate reference is *harder variants*: Scheherazade (arxiv 2410.00151) which algorithmically chains GSM8K problems to test long-range logical integration; GSM-Hard which uses larger numbers; and MATH/MATH-Hard for competition-level math. The signal from these is consistent: models that look "smart" at grade-school arithmetic degrade sharply when the chain of reasoning extends past 4–5 steps without external verification.
...
Anthropic's code execution tool[^6][^7] is, as of April 2026, the most frictionless way to do PAL-style offloading inside a production agent loop. The API exposes a sandboxed Python runtime (and, with the October 2025 `code-execution-2025-08-25` upgrade, bash and file manipulation across multiple languages) that the model can call mid-response. Pricing: 50 free hours per organisation per day, $0.05/hour thereafter; free when bundled with `web_search` or `web_fetch` calls[^8]. OpenAI's Code Interpreter (now Assistants API file search + code execution) is the functional equivalent. LangChain exposes `PythonREPLTool`; for production-grade isolation most teams use a purpose-built sandbox — e2b.dev, Riza, or Modal's serverless containers.
...
But the story does not end there. The same Huang paper is explicit that when *valid external feedback* is available, incorporating it in the self-correction loop produces real gains — code generation with execution results fed back is the canonical example. This is the crucial distinction for analyst-replacement workers: **external feedback works, intrinsic self-reflection does not**. An evaluator-critic loop that has access to a *different signal* than the original generator — a code execution result, a retrieval-grounded fact, a deterministic rule check — is a different architecture than the loop Huang critiqued. It is closer to Wang et al.'s self-consistency[^9] (arxiv 2203.11171; +17.9% on GSM8K over CoT) combined with a verifier.
...
**Anthropic's October 2025 code-execution upgrade (advanced tool use announcement)[^7].** The `code-execution-2025-08-25` release added bash, multi-language, and file manipulation to the Claude sandbox, and Anthropic's engineering post on advanced tool use made the explicit architectural pitch: Claude should be a data analyst that iterates on computations, not a code-writing assistant that hands off. The engineering implication for report-generator builders: the sandbox is now stateful enough across a single turn that you can load a ledger, run fifteen computations, generate a chart, and return a structured result — all without spinning up external infrastructure. This dramatically lowers the build cost of the PAL pattern for small teams.
...
> *"For every question in `eval_set.jsonl`, run the same question through three approaches:
> (A) Claude Opus 4.7 pure CoT, no tools, temperature 0.
> (B) Claude Opus 4.7 with the code execution tool enabled, same prompt, forced tool-use on numerical claims.
> (C) Claude Opus 4.7 generator → critic loop: arm B produces a draft; a second call with the critic prompt (from Layer 4 above) verifies each numerical claim via code execution and returns a structured verdict; if any claim fails verification, re-draft with the verified values.
...
2. **Are frontier reasoning models (o1-class, Claude Opus 4.7 with extended thinking) changing the code-execution calculus?** The early 2025 data is mixed: extended-thinking models seem to close some of the arithmetic gap via longer internal CoT, but they also increase latency and cost to a point where bundling with code execution is still the right default for production. The open question is whether by late 2026 the reasoning models become good enough that inline code execution for arithmetic is *redundant*. I'd bet against, but the gradient is real.
...
[^6]: Anthropic (2025). *Code Execution Tool — Claude API Docs.* platform.claude.com/docs/en/agents-and-tools/tool-use/code-execution-tool and docs.claude.com/en/docs/agents-and-tools/tool-use/code-execution-tool. Supports the Python sandbox description, the `code-execution-2025-08-25` upgrade naming, and the 30-day container retention policy. Verified 2026-04-17.
...
[^8]: Anthropic (2025). *New Capabilities for Building Agents on the Anthropic API.* anthropic.com/news/agent-capabilities-api. Supports the code-execution pricing (50 free hours/day, $0.05/hour thereafter; bundled free with web_search/web_fetch). Verified 2026-04-17.
...
[^12]: Kamoi, R. et al. (2024). *When Can LLMs Actually Correct Their Own Mistakes? A Critical Survey of Self-Correction of LLMs.* TACL 2024, direct.mit.edu/tacl/article/doi/10.1162/tacl_a_00713. Supports the claim that self-correction's efficacy collapses to self-consistency under matched sampling budgets, and the taxonomy of when external-feedback loops do and do not help. Verified 2026-04-17.
