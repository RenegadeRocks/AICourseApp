...
After this lesson you will have shipped — not described — a working AI analyst worker: a pipeline that on a schedule pulls data from a real source, parses whatever documents need parsing, verifies its own numbers through a deterministic runtime, drafts a narrative in a voice you chose, passes the draft through an evaluator-critic, renders the final report, and delivers it with a regression eval gate and an observability trace you can open and read. "You can build that with Claude Code in a weekend" is true and also a trap: the weekend build silently rots in week three when it skips the eval harness, the single-point-of-failure fallback, and the cost model. The delta you will own after this day is the thing that survives that week three.

This is the synthesis day for Week 5. Every earlier lesson was a component: [[01-mon-analyst-replacement-thesis|Monday]] framed the category, [[02-tue-document-understanding-stack|Tuesday]] parsed the documents, [[03-wed-data-connectivity|Wednesday]] wired the connectors, [[04-thu-analytical-reasoning-and-code-offload|Thursday]] hardened the numbers, [[05-fri-report-generation-patterns|Friday]] shaped the generation. Today you compose. The value lives not in any single component — most are fifty lines of orchestration code at this point — but in how you join them, where you insert the gates, and which of the three or four available architectures you commit to. You will finish with a running system and, more importantly, with a defensible position on the two live arguments of 2026: *do you orchestrate on Claude Code or on a framework?* and *do you pay the observability tax on day one?* The rest of Block 2 takes for granted that you have shipped something like this at least once.

## Prerequisites

- The [[04-thu-rag-fundamentals|Week 4]] eval harness pattern (retrieval regression set, LLM-as-judge scoring, hit-rate@k). You are reusing it.
- One real connector from [[03-wed-data-connectivity|Wednesday]]: a **maintained** Postgres MCP server (`crystaldba/postgres-mcp` in `--access-mode=restricted`, *not* the archived reference server), Stripe MCP, a Sheets/Airtable tab, or a dropped-CSV pattern. Anything that returns rows.
- [[04-thu-analytical-reasoning-and-code-offload|Thursday's]] code-execution sandbox enabled in your Claude Code environment *or* an e2b/Modal/Riza endpoint you can call.
...
2. **Narrative quality — LLM-as-judge against rubric.** A current frontier model (parameterise it — Opus 4.8 as of writing) grading each generated report against a 5-point rubric (insight density, voice fidelity, structure, no-hallucination, non-triviality) with the gold report as the calibration example. Use binary scoring per rubric dimension, not 1–5 scores — Arize's 2024/25 experiments and Hamel's pedagogy both converge on binary being more reliable[^4][^6]. For the narrative-quality metric specifically, prefer a *different model family* for the judge than the generator (see the bias caveat below). Regression threshold: 80%+ pass rate on each dimension.
...
**Case 2 — the narrow-workflow vs open-narrative distinction (Brex, Ramp, and now Anthropic's own finance agents).** The disclosed Brex/Ramp numbers are surveyed in full on [[01-mon-analyst-replacement-thesis|Monday]] — the point *here* is the architectural one, not the numbers again. Narrow analyst-replacement workflows (expense categorisation, invoice coding, anomaly flagging, and now Anthropic's month-end-closer / GL-reconciler agent templates, May 2026) are shippable today; open-ended "summarise our business" reports are a different beast. The delta is the input distribution and the feedback signal. Brex's model sees thousands of categorisation decisions with a clear feedback signal from the AP team; a weekly narrative report sees one composition decision per period with no clear feedback signal until a human reads it and frowns — which is exactly why a public AI-analyst-worker postmortem is scarce and why your own build's 30-day human-gate log (below) *is* the missing dataset. Architect for your actual distribution: if your "report" is really a hundred small categorisation decisions stitched together, lean into the Brex/Ramp/Anthropic-template pattern; if it is a single narrative page, lean into the human-gate-then-eval-set pattern above and treat the reader's edits as the feedback signal the categorisation workflows get for free.
...
**Silent drift after model swap.** You upgrade from `claude-sonnet-5` to `claude-opus-4-8` (or to `claude-fable-5`) for quality, and the report's tone shifts noticeably without any prompt change — the newer models also carry a different tokenizer (~30% more tokens for the same text) that shifts your cost math. Caught by running the eval set immediately on model swap and comparing binary rubric pass rates. Fix: re-anchor voice with a style example in the system prompt, or pin the model version in config and gate upgrades behind eval-passing runs.
...
- **Boris Cherny (Head of Claude Code, Anthropic)** would push back on Layer 1's suggestion that LangGraph becomes the right choice at 50 reports/week. His current public record (Fortune, June 2026) is managing hundreds to *tens of thousands* of Claude Code agents at once, hand-writing no code for eight months, with Claude Code writing essentially all of Anthropic's code — session-level and subagent parallelism, not a managed-framework graph[^5]. His counter-position sharpens: the scale threshold for migration is far higher than most people think, and the "parallelism" problem is better solved at the session/subagent level than the graph level — if one engineer orchestrates tens of thousands of agents through Claude Code, "50 reports/week" is nowhere near a framework-forcing scale. The steelmanned response in this lesson is the explicit caveat that for most readers, Claude Code is the correct endpoint, not just the correct start.
...
[^2]: LangChain, *LangGraph 1.0 is now generally available*, changelog entry, October 22 2025. https://changelog.langchain.com/announcements/langgraph-1-0-is-now-generally-available. Current release: LangGraph 1.2.4, June 2, 2026 (per-node timeouts, v2 streaming), https://github.com/langchain-ai/langgraph/releases/tag/1.2.4 and https://www.langchain.com/langgraph. Verified 2026-07-17. Supports: the first stable major release in the durable-agent-framework space; production deployments at Uber, LinkedIn, Klarna; durable execution, built-in persistence, human-in-the-loop, memory management; current 1.2.4 version and ~34k GitHub stars.
...
[^5]: Fortune, "Anthropic's Claude Code creator says there are days he manages tens of thousands of AI agents at once," June 8, 2026, https://fortune.com/2026/06/08/anthropics-boris-cherny-creator-of-claude-code-says-there-are-days-he-manages-tens-of-thousands-of-ai-agents-at-once/; and "The head of Claude Code hasn't 'written a line of code by hand' in 8 months," June 11, 2026, https://fortune.com/2026/06/11/anthropic-claude-boris-cherny-doesnt-write-code-by-hand-anymore/. Verified 2026-07-17. Supports: Boris Cherny's current fleet-scale usage (hundreds to tens of thousands of parallel Claude Code agents), no hand-written code in eight months, Claude Code writing essentially all of Anthropic's code with ~8x volume increase since early 2026, session/subagent-level parallelism over a managed-framework graph. (Supersedes the April draft's "five parallel sessions" community write-up, which understated current scale.)
