...
**Q2** — Category 1 (financial-close automation) — Brex, ~70% of expenses handled entirely by automation, books close 3x faster (Fall 2025 release).[^1] Category 2 (management reporting / CPM) — Pigment, "Analyst Agent" that generates complete narrative reports with charts, KPIs, recommendations (2025 product page). Category 3 (competitive/market intelligence) — Consensus, Klue, CompeteIQ, Crayon, Gong Revenue-AI layer. Category 4 (operational reporting) — no dominant incumbent; Saturday's build targets this.
...
**Q4** — (1) Specialised-pipeline OSS — Unstructured, Docling (IBM, July 2024)[^15], Marker. (2) Hosted gen-AI-first — LlamaParse (Jerry Liu, Feb 2024)[^16], Reducto, Chunkr. (3) Native-multimodal — Claude Vision, GPT-4o/5 Vision, Gemini 2.5 Pro, Qwen-VL.[^17] (4) Cloud document-AI services — AWS Textract, Google Document AI, Azure Form Recognizer. (5) Academic-paper specialists — Nougat (Meta FAIR), GROBID.

**Q5** — Willison's "lethal trifecta" (coined April 2025): an MCP-equipped agent is exploitable when it simultaneously has (a) access to private data, (b) exposure to untrusted content (email bodies, web pages, tickets that an adversary can plant tokens in), and (c) ability to communicate externally. Any one of the three alone is safe-ish; all three together is confused-deputy territory.[^5]
...
**Q15** — Rough costing for Claude Opus 4.7 + code execution + critic loop at typical report sizes (*Opus 4.7 list pricing as of April 2026; re-check before quoting a client*): 1 run/week ≈ $8–$15/run, so ~$35–$65/mo — engineering time dominates. 5 runs/week ≈ $175–$325/mo — still engineering-dominated. 50 runs/week (service bureau) ≈ $1,750–$3,250/mo — now LLM cost is material. The 10x optimisation at 50-run scale: aggressive prompt caching of the common-prompt prefix (schema, tool definitions, critic rubric — often 80% of input tokens), routing simple pages/sections to Claude Haiku, and caching the parsed-markdown of any re-used source document by SHA-256. These three together typically drop cost 8–15x.
...
**Q18** — Refute, with a 2028 hedge. Data points: (a) no BI incumbent shipped narrative-generation that matches mid-market insight-density bar as of April 2026 (Looker's AI features, Tableau Pulse, Metabase AI are descriptive, not insightful); (b) the AI-native entrants (Hex, Mosaic, Pigment) have velocity but not horizontal BI distribution and are moving upmarket into category 2, not down into category 4; (c) category-4 value lives in the voice-transferred narrative layer, which is exactly the layer incumbent BI vendors have the weakest design-taste muscle around. 2028 hedge: if frontier-VLM cost drops another 5x and Tableau/Looker ship voice-transfer-capable narrative pipelines, category 4 compresses. Until then standalone weekly-report-generators have a 24–36 month defensible lane.
...
5. Q: Ramp Intelligence May 17, 2023 disclosed numbers? → A: $163M annualized salary dollars freed; 208,000 hours/month saved across customers.
...
8. Q: Five document-parsing archetypes? → A: (1) Specialised OSS (Unstructured, Docling, Marker), (2) Hosted gen-AI-first (LlamaParse, Reducto, Chunkr), (3) Native-multimodal (Claude Vision, GPT-4o/5, Gemini 2.5 Pro), (4) Cloud document-AI (Textract, Doc AI, Form Recognizer), (5) Academic specialists (Nougat, GROBID).
...
11. Q: LlamaParse launch facts? → A: Jerry Liu, Feb 2024, "first parser built with genAI," schema-guided extraction (LlamaExtract v2, 2025), ~10x cheaper than frontier-VLM-only at comparable robustness on mixed corpora.
...
13. Q: Anthropic native PDF capability? → A: Launched late 2024, full multimodal Feb 2025, URL source blocks May 2025, GA in Claude 4 family; 32 MB / 100-page envelope.
14. Q: MCP launch date and reference servers? → A: Nov 25, 2024; initial servers Google Drive, Slack, GitHub, Git, Postgres, Puppeteer; initial enterprise adopters Block and Apollo.
15. Q: Willison lethal trifecta? → A: Private data access + untrusted content exposure + external communication ability. Any one is safe; all three is exploitable confused-deputy.
...
25. Q: Anthropic code-execution tool pricing? → A: 50 free hours/org/day, $0.05/hour thereafter; free when bundled with `web_search` or `web_fetch`. Upgrade `code-execution-2025-08-25` added bash + multi-language + file manipulation.
...
29. Q: Typical numerical-accuracy stack gain? → A: Single-shot CoT ~68% → generator + code execution ~86% → generator + code execution + critic with forced tool-use ~94% on 30-question weekly-ops eval.
