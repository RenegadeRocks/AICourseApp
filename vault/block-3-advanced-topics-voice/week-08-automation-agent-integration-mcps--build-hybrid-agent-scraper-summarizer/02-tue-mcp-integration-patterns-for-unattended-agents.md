...
3. **Prove idempotency.** Ask Claude to record the same judgment twice, then show `judgments.jsonl`. One record. Now ask it to explain, in two sentences, why this property matters when a cron overlap re-runs the stage. Grade its answer against Layer 3. **Pass bar:** the file holds exactly one record for the repeated `item_id`, and the explanation names both halves — the *cause* (overlap/retry re-executes the stage) and the *consequence prevented* (duplicated side effects or double-paying a judgment stage). Naming only "it might run twice" without what the upsert saves you is a fail.
4. **Audit like OWASP.** *"Against the OWASP Agentic Top 10 (2026), audit this server + session: which ASI risks apply, which are mitigated by construction, which remain?"* Expect it to flag that `record_judgment`'s `reason` field is an exfiltration channel in principle (injected content could be smuggled into judgments) — and decide, in writing, whether you accept that residual risk for a file that only your own pipeline reads. That decision, written down, is a security posture. Congratulations: most teams don't have one. **Acceptance criterion:** the audit classifies at least three applicable ASI risks with an explicit mitigated/residual status each, *and* it (or you, on a second pass) surfaces the `reason`-field seam. An audit that misses the seam is a fail — it's the one channel this architecture leaves open, and finding it is the point of the step. Your written accept/reject decision on that residual risk is the deliverable.
