...
**A6.** A scheduled automation isn't "you at midnight"; it's a service principal with its own identity, scopes, and audit trail. Client credentials authenticate the *application* (client ID + secret) and issue a token representing the application, not a person. That is the correct "no human in the loop" shape.
