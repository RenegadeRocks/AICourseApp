...
Two anchoring disciplines. **Anchor on the buyer's numbers, not yours**: during onboarding, capture the hours-per-week the package displaces, and reuse that number at renewal ("this quarter the Radar covered ~50 analyst-hours"). **Re-anchor at every scope change**: when a customer asks for source #13, quote the labor line of the addition, not just your incremental cost; scope creep priced at labor rates funds itself or stops.
...
**Step 2 — Stress matrix (40 min).** Run all three structures through four scenarios: (i) Sonnet-5 intro expiry (+50% inference COGS); (ii) forced migration to a flagship tier (+150% inference COGS); (iii) a price war (self-serve competitor at 1/5th your Standard price); (iv) a bad month — success rate −15% if outcome-priced, doubled support burden for flat/hybrid structures (the calculator's version). For each cell: gross margin and required response (absorb / reprice / restructure). Claude Code prompt scaffold:
