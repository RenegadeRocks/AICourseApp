...
Tomorrow you ship. Without instrumentation, here is what launch day gives you: a traffic spike, some likes, three nice DMs, and a feeling. The feeling is not data. Next week ([[01-mon-idea-definition-from-itch-to-falsifiable-bet|Week 11]]) runs the formal validation discipline, and it can only run on what launch week *recorded*. Today you wire the page and the channels so that fourteen days from launch you can answer, with numbers you pre-committed to: did the market respond enough to keep investing in this package, and specifically *where* is the response coming from?
